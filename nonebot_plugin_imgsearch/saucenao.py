...
class SauceNao:
    ...
    async def search(self, url: str) -> "Response":
        ...
        * ``proxy: str`` proxy (default: None , edit in .env file)
        ...
        url_all = (
            "http://saucenao.com/search.php?output_type=2&numres=1&minsim="
            + default_minsim
            + "&db="
            + bitmask_all
            + "&api_key="
        ...
        for i in range(3):
            try:
                async with httpx.AsyncClient(proxies=self.proxy) as client:
                    r = await client.post(
                        url=url_all, files=files, follow_redirects=True
                    )
                break
        ...
        if r.status_code != 200:
            if r.status_code == 403:
                ...
                return Response(
                    ACTION_FAILED, message="其他错误, Error Code: " + str(r.status_code)
                )
        else:
            ...
            if int(results["header"]["user_id"]) > 0:
                ...
                if int(results["header"]["status"]) == 0:
                    ...
                    if int(results["header"]["status"]) > 0:
                        return Response(
                            ACTION_FAILED,
                            message="SauceNAO服务器部分索引出现问题, 请稍后再试",
                        )
                    else:
                        return Response(
                            ACTION_FAILED, message="图片无法读取或其他请求错误"
                        )
            else:
            ...
            if int(results["header"]["results_returned"]) > 0:
                ...
                if float(results["results"][0]["header"]["similarity"]) > float(
                    ...
                    if index_id == 5 or index_id == 6:
                        ...
                        found_json["data"] = {"author": member_id, "illust": illust_id}
                    elif index_id == 9:
                        # 9 -> danbooru
                        ...
                        return Response(
                            ACTION_FAILED,
                            message=f"Unhandled Index {index_id}, check log for more infomation.",
                        )
                    ...
                    return Response(
                        ACTION_WARNING, message=f"rate: {rate}\nnot found... ;_;"
                    )
