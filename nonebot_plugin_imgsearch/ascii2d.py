...
class SingleRes:
    def __init__(
        self, title=None, title_url=None, author=None, author_url=None, thumb_url=None
    ):
        self.title = title
    ...
    async def search(self, url: str) -> "BaseResponse":
        try:
            ...
            color_response = await client.get(
                "https://ascii2d.net/search/url/" + url, follow_redirects=True
            )
            ...
            color_results[1].thumbnail = (
                await client.get(color_results[1].thumbnail_url)
            ).content
            bovw_results[1].thumbnail = (
                await client.get(bovw_results[1].thumbnail_url)
            ).content
            ...
            return BaseResponse(
                ACTION_FAILED, message="连接代理服务器出现错误, 请检查代理设置"
            )
